...
function InfoDrop({titleInfo, descriptionInfo, descHeight}) {
  ...
  return (
    <div className={`my-3 w-full text-black text-xl seconde:text-2xl hover:bg-gray-100 bg-gray-50 rounded-md pl-2 pr-4 ${opened && 'bg-gray-50'}`}>
        <div className='flex items-center justify-between'  onClick={openInfo}><h1 className=''>{titleInfo}</h1> <motion.img animate={{rotate : opened ? 270 : 90}} src="/icons/arrow-icon.svg" className='rotate-90' alt="" /></div>
